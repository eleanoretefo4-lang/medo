...
const navItems: NavItem[] = [
  { label: 'HOME', to: '/', variant: 'green' },
  { label: 'OUR WORKS', to: '/works', variant: 'pink' },
  { label: 'SKILLS', to: '/skills', variant: 'green' },
  { label: 'ABOUT', to: '/about', variant: 'pink' },
  { label: 'CONTACT', to: '/contact', variant: 'green' },
  { label: 'LOGIN', to: '/login', variant: 'pink' },
];
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/30 backdrop-blur-md shadow-lg shadow-black/20">
          <div className="flex items-center justify-between px-4 py-3 md:px-6">
            ...
            <ul className="hidden md:flex items-center gap-3 text-sm font-semibold">
              {navItems.map((item) => (
                <li key={item.to}>
                  <NavNeonButton to={item.to} variant={item.variant}>{item.label}</NavNeonButton>
                </li>
              ))}
            </ul>
          </div>
          ...
          {open && (
            <div className="md:hidden px-4 pb-4">
              <ul className="grid gap-2">
                {navItems.map((item) => (
                  <li key={item.to}>
                    <NavNeonButton to={item.to} variant={item.variant} className="w-full justify-center" onClick={() => setOpen(false)}>
                      {item.label}
                    </NavNeonButton>
                  </li>
                ))}
